function [P,V]=TpfaTwoPhase(Grid,K,q,P,dt)
% function [P,V,rhs]=TPFA(Grid,K,G,q)
...
% Construct the interblock Gravity transmissibility vector 
% H = G.^(-1);
% TG = zeros(Nx,Ny,Nz+1);
% TG(:,:,2:Nz) = tz./(H (:,:,1:Nz-1)+H(:,:,2:Nz));

% Add the gravity transmissibility vector to system RHS.
% diff_g = zeros(Nx,Ny,Nz); diff_g(:,:,1:Nz) = -TG(:,:,2:Nz+1) + TG(:,:,1:Nz);
% diff_g = reshape(diff_g,N,1);
% rhs = zeros(N,1);
% rhs = q;
% for i=1:N
%    if ( q(i)==0 ) rhs(i) = q(i) + diff_g(i); end 
% end
